...
export default function WebcamFeed({ webcam, timestamp }) {
  ...
  return (
    <a href={webcam.sourceUrl} target="_blank" rel="noopener noreferrer"
      className="group rounded-2xl bg-bg-card ring-1 ring-border hover:ring-cyan/40 overflow-hidden card-hover flex flex-col cursor-pointer transition-all">
      <div className="relative bg-bg-elevated aspect-video flex items-center justify-center overflow-hidden">
        {!loaded && !error && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-6 h-6 rounded-full border-2 border-border border-t-cyan/50 animate-spin" />
          </div>
        )}
        {error ? (
          <div className="flex flex-col items-center gap-2 p-4 text-center">
            <span className="text-2xl opacity-30">📷</span>
            <p className="text-text-muted text-xs">Camera offline</p>
            <span className="text-cyan text-xs underline underline-offset-2">
              View at source →
            </span>
          </div>
        ) : (
      ...
      <div className="px-4 py-3 border-t border-border-subtle">
        <p className="text-text-primary text-xs font-semibold">{webcam.label}</p>
        <p className="text-text-muted text-xs mt-0.5">{webcam.sublabel}</p>
      </div>
    </a>
  );
}
